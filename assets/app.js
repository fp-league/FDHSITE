...
function renderAuthState(){
  ...
  const loggedOutBtMsg = $('loggedOutBtMsg');
  if(loggedOutBtMsg) loggedOutBtMsg.classList.toggle('hidden', isLoggedIn);
  const btTimeInput = $('btTimeInput');
  if(btTimeInput) btTimeInput.disabled = !isLoggedIn;
...
// ---------- Best Times ----------
// Times are stored as raw milliseconds (integer) so they sort/compare correctly,
// but entered and displayed as M:SS.mmm (e.g. 1:23.456).

function parseLapTime(str){
  // Accepts "1:23.456", "83.456", "1:23", etc. Returns milliseconds, or null if invalid.
  str = (str || '').trim();
  if(!str) return null;
  let minutes = 0, secondsPart = str;
  if(str.includes(':')){
...
// ----- Public tracks list (besttimes.html) -----
async function loadBestTimesTracks(){
  const grid = $('btTracksGrid');
  if(!grid) return;

  const snap = await db.collection('bt_tracks').where('active', '==', true).orderBy('name').get();
  const tracks = snap.docs.map(d => ({ id: d.id, ...d.data() }));

  if(!tracks.length){
    grid.innerHTML = `<div class="promo-empty">No tracks yet — check back soon.</div>`;
    return;
  }

  grid.innerHTML = tracks.map(t => `
    <a href="track.html?id=${t.id}" class="bt-track-card">
      ${t.image_url ? `<div class="bt-track-img" style="background-image:url('${t.image_url}')"></div>` : `<div class="bt-track-img bt-track-img-empty">🏁</div>`}
      <div class="bt-track-body">
        <h3>${t.name}</h3>
        ${t.map_code ? `<div class="bt-track-code">${t.map_code}</div>` : ''}
      </div>
    </a>`).join('');
}

// ----- Track detail page (track.html) -----
async function loadTrackDetail(){
  const wrap = $('trackDetailWrap');
  if(!wrap) return;
  ...
  const doc = await db.collection('bt_tracks').doc(id).get();
  if(!doc.exists){ if(notFound) notFound.classList.remove('hidden'); return; }
  const t = doc.data();

  if($('tdName')) $('tdName').textContent = t.name;
  if($('tdDesc')) $('tdDesc').textContent = t.description || '';
  if($('tdCode')){
    if(t.map_code){
      $('tdCode').textContent = t.map_code;
      if($('tdCodeRow')) $('tdCodeRow').classList.remove('hidden');
    }
  }
  if($('tdImage') && t.image_url){
    $('tdImage').style.backgroundImage = `url('${t.image_url}')`;
    $('tdImage').classList.remove('hidden');
  }

  // Pre-fill the hidden track id on the submit form
  if($('btSubmitTrackId')) $('btSubmitTrackId').value = id;

  // Load approved times for this track, fastest first
  const timesSnap = await db.collection('bt_times')
    .where('track_id', '==', id)
    .where('status', '==', 'approved')
    .get();
  const times = timesSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  times.sort((a, b) => a.time_ms - b.time_ms);

  const board = $('tdLeaderboard');
  const empty = $('tdLeaderboardEmpty');
  if(board){
    if(!times.length){
      board.innerHTML = '';
      if(empty) empty.classList.remove('hidden');
...
async function submitBestTime(){
  const msg = $('btSubmitMsg');
  if(msg){ msg.className = 'form-msg'; msg.textContent = ''; }
  if(!currentUser || !currentProfile){
    if(msg){ msg.textContent = 'Log in to submit a time.'; msg.className = 'form-msg error'; }
    return;
  }

  const trackId = $('btSubmitTrackId').value;
  const raw = $('btTimeInput').value;
  const ms = parseLapTime(raw);
  if(ms == null){
    if(msg){ msg.textContent = 'Enter a valid time like 1:23.456'; msg.className = 'form-msg error'; }
    return;
  }

  try{
    await db.collection('bt_times').add({
      track_id: trackId,
      uid: currentUser.uid,
    ...
    if(msg){ msg.textContent = e.message; msg.className = 'form-msg error'; }
    return;
  }

  if(msg){ msg.textContent = 'Submitted — waiting on admin approval.'; msg.className = 'form-msg ok'; }
  $('btTimeInput').value = '';
}

// ----- Admin: track management + time approval (admin-times.html) -----
async function loadAdminTimes(){
  if(!$('adminTracksMgmtTable')) return;

  // Tracks table
  const trackSnap = await db.collection('bt_tracks').orderBy('name').get();
  const tracks = trackSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  const trackBody = document.querySelector('#adminTracksMgmtTable tbody');
  trackBody.innerHTML = '';
  tracks.forEach(t => {
    trackBody.innerHTML += `
      <tr>
        <td>${t.name}</td>
        <td class="mono">${t.map_code || '—'}</td>
        <td class="${t.active ? 'tag-active' : 'tag-inactive'}">${t.active ? 'Active' : 'Off'}</td>
        <td><button class="row-btn" onclick="toggleBtTrack('${t.id}', ${t.active})">${t.active ? 'Deactivate' : 'Activate'}</button></td>
        <td><button class="row-btn danger" onclick="deleteBtTrack('${t.id}', '${(t.name||'').replace(/'/g, "\\'")}')">Delete</button></td>
      </tr>`;
  });

  // Build a track-id -> name lookup for the times tables
  const trackName = {};
  tracks.forEach(t => { trackName[t.id] = t.name; });

  // Pending times
  const pendingSnap = await db.collection('bt_times').where('status', '==', 'pending').get();
  const pending = pendingSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  pending.sort((a, b) => (a.submitted_at || '').localeCompare(b.submitted_at || ''));
  const pendBody = document.querySelector('#adminPendingTimesTable tbody');
  const pendEmpty = $('adminPendingTimesEmpty');
  pendBody.innerHTML = '';
  if(pending.length){
    if(pendEmpty) pendEmpty.classList.add('hidden');
    pending.forEach(t => {
      pendBody.innerHTML += `
        <tr>
          <td>${trackName[t.track_id] || '(deleted track)'}</td>
          <td>${t.callsign || '—'}</td>
          <td class="mono">${formatLapTime(t.time_ms)}</td>
          <td>
            <button class="approve-btn" onclick="reviewBtTime('${t.id}','approved')">Approve</button>
            <button class="reject-btn" onclick="reviewBtTime('${t.id}','rejected')">Reject</button>
          </td>
        </tr>`;
    });
  } else {
    if(pendEmpty) pendEmpty.classList.remove('hidden');
  }

  // Approved times (so admin can delete them)
  const apprSnap = await db.collection('bt_times').where('status', '==', 'approved').get();
  const approved = apprSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  approved.sort((a, b) => a.time_ms - b.time_ms);
  const apprBody = document.querySelector('#adminApprovedTimesTable tbody');
  const apprEmpty = $('adminApprovedTimesEmpty');
  apprBody.innerHTML = '';
  if(approved.length){
    if(apprEmpty) apprEmpty.classList.add('hidden');
    approved.forEach(t => {
      apprBody.innerHTML += `
        <tr>
          <td>${trackName[t.track_id] || '(deleted track)'}</td>
          <td>${t.callsign || '—'}</td>
          <td class="mono">${formatLapTime(t.time_ms)}</td>
          <td><button class="row-btn danger" onclick="deleteBtTime('${t.id}')">Delete</button></td>
        </tr>`;
    });
  } else {
    if(apprEmpty) apprEmpty.classList.remove('hidden');
  }

  // Populate the "add time manually" track dropdown
  const sel = $('adminAddTimeTrack');
  if(sel){
...
async function addBtTrack(){
  const name = $('newTrackName2').value.trim();
  const description = $('newTrackDesc').value.trim();
  const map_code = $('newTrackCode').value.trim();
  const image_url = $('newTrackImage').value.trim();
  const msg = $('addTrackMsg');
  if(msg){ msg.className = 'form-msg'; msg.textContent = ''; }
  if(!name){ if(msg){ msg.textContent = 'Track name is required.'; msg.className = 'form-msg error'; } return; }
  if(image_url && !/^https?:\/\//i.test(image_url)){
    if(msg){ msg.textContent = 'Image must be a URL starting with http:// or https://'; msg.className = 'form-msg error'; } return;
  }

  await db.collection('bt_tracks').add({ name, description, map_code, image_url, active: true, created_at: new Date().toISOString() });
  $('newTrackName2').value = ''; $('newTrackDesc').value = ''; $('newTrackCode').value = ''; $('newTrackImage').value = '';
  if(msg){ msg.textContent = 'Track added.'; msg.className = 'form-msg ok'; }
  loadAdminTimes();
}

async function toggleBtTrack(id, active){
  await db.collection('bt_tracks').doc(id).update({ active: !active });
  loadAdminTimes();
}

async function deleteBtTrack(id, name){
  if(!confirm(`Delete track "${name}"? This removes the track. Its submitted times will remain in the database but won't show anywhere. This can't be undone.`)) return;
  await db.collection('bt_tracks').doc(id).delete();
  loadAdminTimes();
}

async function reviewBtTime(id, status){
  await db.collection('bt_times').doc(id).update({ status, reviewed_at: new Date().toISOString() });
  loadAdminTimes();
}

async function deleteBtTime(id){
  if(!confirm('Delete this time permanently?')) return;
  await db.collection('bt_times').doc(id).delete();
  loadAdminTimes();
}

async function adminAddTime(){
  const trackId = $('adminAddTimeTrack').value;
  const callsign = $('adminAddTimeCallsign').value.trim();
  const raw = $('adminAddTimeValue').value;
  const msg = $('adminAddTimeMsg');
  if(msg){ msg.className = 'form-msg'; msg.textContent = ''; }

  if(!trackId){ if(msg){ msg.textContent = 'Pick a track.'; msg.className = 'form-msg error'; } return; }
  if(!callsign){ if(msg){ msg.textContent = 'Enter a callsign.'; msg.className = 'form-msg error'; } return; }
  const ms = parseLapTime(raw);
  if(ms == null){ if(msg){ msg.textContent = 'Enter a valid time like 1:23.456'; msg.className = 'form-msg error'; } return; }
  ...
  await db.collection('bt_times').add({
    ...
    callsign,
    time_ms: ms,
    status: 'approved', // admin-added times are pre-approved
    submitted_at: new Date().toISOString(),
    reviewed_at: new Date().toISOString()
  });

  $('adminAddTimeCallsign').value = ''; $('adminAddTimeValue').value = '';
  if(msg){ msg.textContent = 'Time added.'; msg.className = 'form-msg ok'; }
  loadAdminTimes();
}
